...
game_events_agg as (
    select
        ...
        sum(case
            when
                ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'home'
                then penalty_duration
        end) as home_penalties,
        sum(case
            when
                ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'away'
                then penalty_duration
        end) as away_penalties,
        ...
        count(case
            when
                event_description = 'giveway'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'home'
                then event_id
        end) as home_giveaways,
        count(case
            when
                event_description = 'giveway'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'away'
                then event_id
        end) as away_giveaways,

        count(case
            when
                event_description = 'takeway'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'home'
                then event_id
        end) as home_takeaways,
        count(case
            when
                event_description = 'takeway'
                and ge.event_owner_team_id = gn.team_id
        ...
        count(case
            when
                ...
                and gn.home_or_away_flag = 'away'
                then event_id
        end) as away_blocked_shots,

        count(case
            when
                event_description = 'missed-shot'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'home'
                then event_id
        end) as home_missed_shots,
        count(case
            when
                event_description = 'missed-shot'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'away'
                then event_id
        end) as away_missed_shots,

        count(case
            when
                event_description = 'faceoff'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'home'
                then event_id
        end) as home_faceoff_won,
        count(case
            when
                event_description = 'faceoff'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'away'
                then event_id
        end) as away_faceoff_won,

        count(case
            when
                event_description = 'failed-shot-attempt'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'home'
                then event_id
        end) as home_failed_shot_attempt,
        count(case
            when
                event_description = 'failed-shot-attempt'
                and ge.event_owner_team_id = gn.team_id
                and gn.home_or_away_flag = 'away'
                then event_id
        end) as away_failed_shot_attempt
    from game_events as ge
    left join games_norm as gn
